...
const metaApiVersion = defineSecret("META_API_VERSION");
const metaWabaId = defineSecret("META_WABA_ID");
const adminAccessHash = defineSecret("ADMIN_ACCESS_HASH");
 ...
 */
async function authenticateCaller(token?: string): Promise<AuthenticatedCaller | null> {
 ...
 */
function sanitizePhone(phone: string): { valid: boolean; cleanPhone: string; error?: string } {
 ...
 */
export const getWhatsAppConfigStatus = onRequest(
  {
 ...
 */
export const sendWhatsAppTestMessage = onRequest(
  {
  ...
  async (req, res) => {
    ...
    const cleanParams: string[] = Array.isArray(parameters)
